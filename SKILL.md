...
8. **Optional post-production.** Use `postproduction/remotion-viral/` to layer brief glitch/static accents at clip breaks, color-wash titles at major topic cuts, and procedural FFmpeg sounds beneath the untouched narration. For explanation-heavy narration, add concise typed capability cards keyed to exact SRT start/end times: category kicker, one large headline, and optional bullets that reveal in spoken order. Treat these as presentation slides, not full captions; keep 9:16 safe margins, use 55–85px headline type, limit holds to roughly 2.2–4.8 seconds, and avoid major-title/CTA collisions. Render representative card stills first, then the complete composition in 600–900-frame low-concurrency chunks. Completion criterion: sampled frames from the actual final MP4 visibly show the programmed cards throughout the timeline, narration remains intelligible, and final duration/frame count are unchanged.
